...
Views.define("recommend",{className:"schedule",initialize:function(){App.foursquare_token||App.authenticate_foursquare();this.attach_behavior("place_selector");this.place_search_path="/users/self/checkins"},render:function(){this.$el.html($(Templates.recommend).html());this.$el.find("strong").html(App.meeting.user.name);return this},before_transition:function(){var a=this;this.$el.find("p.start a").link({run:function(){a.send_recs()}})},after_transition:function(){this.run("refresh_places",["",{}])},
use_place:function(a){a.parents("li").first().toggleClass("selected")},send_recs:function(){var a=this;App.api.request("put@meetings/"+App.meeting.id,{data:{recommendations_attributes:$.map(this.$el.find("ul.places li.selected a"),function(b){return{foursquare:a.places[$(b).attr("data-id")]}})},success:function(){console.log(this)}})}});
Behaviors.place_selector={initialize:function(){this.places={}},refresh_places:function(a,b){var c=this,d=$('<span class="bjr_loader"><span></span></span>');d.css({position:"absolute",top:160,left:"50%","margin-left":-14});this.$el.find("ul.places").empty().before(d);App.get_position(function(b){var d={ll:b.latitude+","+b.longitude,v:"20130823",limit:200};c.position={lat:b.latitude,lng:b.longitude};App.foursquare_token?d.oauth_token=App.foursquare_token:(d.client_id=App.params.foursquare.client_id,
d.client_secret="CAHKQWBRYE3W33VLHFNLAUEEMZJ5H124JDCXSPZW0SUORM0P");a&&0<a.length&&(d.query=a);c.requests.push($.ajax({url:"https://api.foursquare.com/v2"+(c.place_search_path||"/venues/search"),data:d,dataType:"json",success:function(a){a.response.venues&&0<a.response.venues.length?c.run("display_places",[a.response.venues]):a.response.checkins&&0<a.response.checkins.items.length&&c.run("display_places",[$.map(a.response.checkins.items,function(a){return a.venue})])}}))})},close_selector:function(a){var b=
