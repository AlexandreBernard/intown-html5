...
["th","st","nd","rd"],g=b%100;return b+(a[(g-20)%10]||a[g]||a[0])},date_number:function(b){return 1==b.toString().length?"0"+b:b},get_position:function(b){navigator.geolocation?navigator.geolocation.getCurrentPosition(function(a){b({latitude:a.coords.latitude,longitude:a.coords.longitude})},function(a){b({latitude:null,longitude:null})}):b(!1)}};Date.prototype.addDays=function(b){var a=new Date(this.valueOf());a.setDate(a.getDate()+b);return a};$.fn.prop=$.fn.prop||$.fn.attr;
String.prototype.trim=function(){return this.replace(/^\s+/,"").replace(/\s+$/,"")};$(document).ready(function(){App.initialize()});
...
display_places:function(b){for(var a=this.$el.find("ul.places"),g=this,c=0;c<b.length;c++){var d=$('<li><a href="#" data-id="'+b[c].id+'"></a></li>');d.find("a").append("<strong>"+b[c].name+"</strong>").append("<span>"+this.behaviors.place_selector.foursquare_location(b[c])+"</span>");a.append(d);d.find("a").link({run:function(a){a.preventDefault();g.run("use_place",[this])}});this.places[b[c].id]=b[c]}Nav.run("resize")},foursquare_location:function(b){var a="";b.location.address&&(a+=b.location.address+
", ");b.location.postalCode&&(a+=b.location.postalCode+" ");b.location.city&&(a+=b.location.city);return a}};
var Templates={complete:'<div class="complete"><div class="message"><p><strong>{name}</strong> thanks you!</p><p>You\'re gonna receive news from him soon.</p></div></div>',decline:'<div class="complete"><div class="message"><p><strong>{name}</strong> will be missing you.</p><p>Too bad.</p></div></div>',error:'<div><div class="complete"><div class="message"><p><strong>There was a problem</strong></p><p>This meeting doesn\'t exist or a problem occured.</p></div></div></div>',invitation:'<div class="invitation"><p class="title">Your friend <strong>{name}</strong> is visiting town and wants to meet you!</p><div class="info"><p class="location">{location}</p><p class="time"></p><p class="start"><a href="/schedule">Let\'s meet him</a></p></div></div>',
place:'<div class="schedule"><div class="title"><p>Suggest a nice place to meet</p><nav class="steps"><span class="done"></span><span class="done"></span><span class="on"></span></nav></div><div class="place"><div class="current_location"><p><a href="#">At my current location</a></p></div><form class="foursquare"><p class="search"><input name="q" placeholder="In a public location" type="search"/></p><p class="send"><input type="submit" value="Ok"/></p><ul class="places"></ul><p class="powered">Powered by Foursquare</p></form></div><div class="info"><p class="start"><a href="/place">Finish</a></p></div></div>',
